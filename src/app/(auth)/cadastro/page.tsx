...
export default function RegisterPage() {
  ...
  async function onSubmit({ name, email, password }: RegisterForm) {
    ...
    try {
      ...
      router.replace(roleToDashboardPath(session.user_role));
      router.refresh();
    } catch (error) {
      setFormError(
      ...
      >
        <div className="mb-10 text-center sm:mb-12">
          ...
          <p className="mt-3 text-sm leading-6 text-gray-500">
            Salve suas escolhas, monte seu carrinho e acompanhe seus pedidos.
          </p>
        </div>
        ...
        >
          ...
          >
            {isSubmitting ? (
              <>
                <LoaderCircle className="animate-spin" /> Criando sua conta...
              </>
            ) : (
              "Criar conta"
            )}
        ...
        <div className="my-7 flex items-center gap-3 text-xs text-gray-500 before:h-px before:flex-1 before:bg-gray-200 after:h-px after:flex-1 after:bg-gray-200">
          ou
        </div>
        ...
        <p className="mt-10 flex items-center justify-center gap-2 text-center text-xs text-gray-500">
          <LockKeyhole aria-hidden="true" className="size-3.5" /> Seus dados são
          protegidos.
        </p>
      </section>
    </main>
