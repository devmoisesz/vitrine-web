...
function StoreProductsPageContent({
  ...
  const basePath = `/loja/${slug}/produtos`;
  const hasFilters = Boolean(
    query.name || query.categoryId || query.subcategoryId,
  );
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <main className="mx-auto w-full max-w-[1400px] px-4 py-8 md:px-8 md:py-12">
        {profile.data && <StoreProductsBar store={profile.data} slug={slug} />}
        <div className="mb-6 md:hidden">
          <CategoryChips basePath={basePath} />
        </div>
        <div className="flex gap-10">
          <aside className="hidden w-60 shrink-0 md:block">
            <div className="sticky top-24">
              <CategorySidebar basePath={basePath} />
            </div>
          </aside>
          <div ref={gridRef} className="min-w-0 flex-1">
            <ProductGrid
              ...
              emptyMessage={
                hasFilters ? undefined : "Esta loja ainda não tem produtos."
              }
            ...
            {(products.data?.data.length ?? 0) > 0 && (
              <Pagination
                currentPage={page}
                itemsInCurrentPage={products.data?.data.length ?? 0}
                gridRef={gridRef}
                basePath={basePath}
              />
            )}
