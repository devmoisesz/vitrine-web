...
export function StoreLogoManager({ logoUrl, slug, accessToken }: { logoUrl: string | null; slug: string; accessToken: string }) {
  ...
  return <section className="rounded-xl border border-gray-200 bg-white p-5 sm:p-7"><h2 className="font-serif text-2xl">Logo</h2><p className="mt-1 text-sm text-gray-500">PNG, JPG ou WebP, com no máximo 5MB.</p><div className="mt-5 flex flex-wrap items-center gap-4">{logoUrl ? <Image src={logoUrl} alt="Logo atual da loja" width={96} height={96} className="size-24 rounded-lg border border-gray-200 object-cover" /> : <div className="flex size-24 items-center justify-center rounded-lg border border-dashed border-gray-300 text-xs text-gray-500">Sem logo</div>}<input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(event) => void selectFile(event.target.files?.[0])} disabled={pending} /><Button onClick={() => inputRef.current?.click()} disabled={pending}>{pending && !remove.isPending ? "Enviando..." : logoUrl ? "Trocar" : "Enviar logo"}</Button>{logoUrl ? <Button variant="secondary" onClick={() => void deleteLogo()} disabled={pending}>{remove.isPending ? "Removendo..." : "Remover"}</Button> : null}</div>{error ? <p className="mt-3 text-sm font-medium">{error}</p> : null}</section>;
}
