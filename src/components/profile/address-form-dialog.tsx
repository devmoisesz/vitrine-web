...
import { useSaveAddress } from "@/features/profile/hooks/use-save-address";
...
export function AddressFormDialog({
  ...
  useEffect(() => {
    if (open)
      form.reset(
        address ? { ...address, complement: address.complement ?? "" } : empty,
      );
  }, [open, address, form]);
  ...
  async function lookupCep(value: string) {
    ...
    try {
      ...
      form.setValue("street", result.logradouro ?? "", { shouldDirty: true });
      form.setValue("neighborhood", result.bairro ?? "", { shouldDirty: true });
      form.setValue("city", result.localidade ?? "", { shouldDirty: true });
      form.setValue("state", result.uf ?? "", { shouldDirty: true });
    } catch (error) {
      form.setError("cep", {
        message:
  ...
  async function submit(values: Values) {
    try {
      ...
      onOpenChange(false);
      onSuccess(address ? "Endereço atualizado." : "Endereço adicionado.");
    } catch (error) {
      form.setError("root", {
        message:
          error instanceof Error
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Viewport className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <Dialog.Popup className="max-h-[90vh] w-full max-w-2xl overflow-y-auto bg-background p-5 shadow-xl md:p-7">
            ...
            >
              ...
              {field("street", "Rua", { className: "md:col-span-2" })}
              {field("number", "Número")}
