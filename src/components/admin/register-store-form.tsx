...
const registerStoreSchema = z.object({
  ...
  store_email: z
    ...
    .refine(
      ...
      "Insira um e-mail de loja válido.",
    ),
  owner_email: z
    .string()
    .trim()
  ...
  whatsapp: z
    ...
    .refine(
      ...
      "O número de WhatsApp deve conter DDD e um número válido.",
    ),
});
...
export function RegisterStoreForm() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      if (/propriet|owner|usuário|usuario|user|conta/i.test(message))
        setError("owner_email", {
          type: "server",
          message:
            "Nenhuma conta encontrada com este e-mail. O proprietário precisa se cadastrar antes.",
        });
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="max-w-xl space-y-5 rounded-xl border border-gray-200 bg-white p-5 sm:p-7"
      noValidate
    >
      {apiError ? (
        <div
      ...
      <Field label="Nome da loja *" error={errors.store_name?.message}>
        <Input {...register("store_name")} autoComplete="organization" />
      </Field>
      <Field
        label="E-mail da loja (opcional)"
        error={errors.store_email?.message}
      >
        <Input {...register("store_email")} type="email" autoComplete="email" />
      </Field>
      <Field
        label="E-mail do proprietário *"
        error={errors.owner_email?.message}
      >
        <Input {...register("owner_email")} type="email" autoComplete="email" />
      ...
      >
        {mutation.isPending ? "Cadastrando..." : "Cadastrar loja"}
      </Button>
    </form>
  );
}
