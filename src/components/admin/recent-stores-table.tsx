          ...
          <table className="w-full min-w-150 text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
              <tr>
                ...
                <th className="px-5 py-3 font-medium">E-mail de contato</th>
                <th className="px-5 py-3 font-medium">Criada em</th>
              </tr>
            </thead>
            <tbody>
              {data.map((store) => (
                <tr key={store.id} className="border-t border-gray-200">
  ...
  return (
    <div className="space-y-px p-5">
      {Array.from({ length: 5 }).map((_, index) => (
        <div
          key={index}
          className="grid grid-cols-3 gap-8 border-b border-gray-100 py-4"
        >
